...
<#
.SYNOPSIS
   Function to stop multiple VMs simultaneously
.DESCRIPTION
   This Azure Automation function uses the awesome Invoke-Parallel Powershell Function from Cookie.Monster to automate simultaneous VMs shutdown.
   https://github.com/RamblingCookieMonster/Invoke-Parallel 
   The Invoke-Parallel function runs the shutdown function in a Script Block by opening multiple runspaces/threads.
   Based on test runs, it is faster than using Foreach -Parallel in a workflow. The function can be deployed as Runbook
   and triggered by a set schedule or a webhook. It can take a Resource Group name or csv file of Virtual Machines as parameters.
...
.EXAMPLE
        Stop-VMsRunbook -FilePath "C:\MeterRates\VSProtestvms.csv"

            Runs against a csv file specified with the FilePath parameter
.EXAMPLE
        Stop-VMsRunbook -ResourceGroupName rglab
...
elseif ($resourceGroupName) {
    ...
    "Please enter a vlaue for FilePath or ResourceGroupName"
}
